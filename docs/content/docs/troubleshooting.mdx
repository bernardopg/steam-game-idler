...
**Symptoms:** SGI asks for your Steam API key or card-farming credentials again after upgrading.

**Solutions:**
- Start the updated app once and allow it to finish opening. SGI automatically imports settings and sign-in state from the previous application data location when needed.
- Do not delete either application-data folder while diagnosing an update. The migration is one-time, preserves values already saved by the new version, and does not import old log history.
- If credentials are still missing, close SGI and include only non-sensitive log timestamps and application version when opening an issue. Never include your API key, `sessionid`, or `steamLoginSecure` values.
...
- Delete the games's achievement cache file and try again:
    1. Close SGI
    2. Navigate to `%appdata%/roaming/com.github.bernardopg.SteamGameIdler/cache/{user_id}/achievement_data`
...
<Steps>
    ...
    2. Navigate to `%appdata%/roaming/com.github.bernardopg.SteamGameIdler`
    ...
    ### Report Issues
    If you find persistent errors, include relevant log excerpts when [reporting issues on GitHub](https://github.com/bernardopg/steam-game-idler/issues/new?template=issue_report.yml)
</Steps>
